...
async function run() {
  ...
  try {
    ...
    const res = await fetch(
      `https://api.meetup.com/BitDevsNYC/events?&sign=true&photo-host=public&page=${LIMIT}&status=past`,
    );
  ...
  const results = await Promise.all(
    events.map(async (e) => {
      ...
      const eventType =
        e.name.indexOf("Whitepaper") !== -1 ? "whitepaper" : "socratic";
      const markdown = turndown
        .turndown(e.description)
        .replace(/\[masked\]/g, "");
