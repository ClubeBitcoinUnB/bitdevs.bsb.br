---
layout: post
type: socratic
title: "Seminário Socrático 024"
meetup: https://evento.so/p/evt_KfXGi6EfbttacxHd
---

## Avisos

* Entrem no grupo do Whatsapp "[BitdevsBSB](https://chat.whatsapp.com/KxuGyYu4TZy94KcA1yXCzi)" para ajudar na curadoria dos encontros!
* Siga Bitdevs Brasília no 
    * [Twitter](https://twitter.com/BitDevsBSB), e 
    * Nostr: npub1gttp04qscpuau2220hpave59dxtjj4put900gxw3y5jje7ugkzcqgml8r4

* Novos temas podem ser sugeridos nos [issues do GitHub do grupo](https://github.com/BitDevsBSB/BitDevsBSB/issues).
* Respeite a privacidade dos participantes.
* Os meetups nunca são gravados. Queremos todos a vontade para participar e discutir os assuntos programados, de forma anônima se assim o desejarem.
* [Livro - 101 Perguntas sobre Bitcoin](https://bitcoin101.site)
* [Clube Bitcoin UnB](https://x.com/ClubeBitcoinUnB)
* [Curso de Bitcoin do Edil](https://www.youtube.com/watch?v=gCgdCgyHFqw&list=PLfdR3_dt2rbexb-ohbaLLzAuNAp7Ypt8u)

## Agradecimentos

* Agradecemos à Vinteum por patrocinar as comidas e bebidas.

## Cronograma

### Aquecimento
* [Bitcoin Optech newsletter #384](https://bitcoinops.org/en/newsletters/2025/12/12/)


### Bitcoin L1
* [Bitcoin Core version v30.1rc1](https://github.com/bitcoin/bitcoin/blob/v30.1rc1/doc/release-notes.md)
* [LNHANCE a soft-fork package](https://groups.google.com/g/bitcoindev/c/AlMqLbmzxNA?pli=1)
* [BIP-337: fix incorrect reference](https://github.com/bitcoin/bips/pull/2053)
* [Why is the miniscript after value limited at 0x80000000?](https://bitcoin.stackexchange.com/questions/129253/why-is-the-miniscript-after-value-limited-at-0x80000000)


### Lightning e L2
* [lnd v0.20 released](https://x.com/roasbeef/status/1998533328274862139)
* [LDK Node 0.7.0 released](https://newreleases.io/project/github/lightningdevkit/ldk-node/release/v0.7.0)


### Mineração
* [Stratum v2 updates](https://bitcoinops.org/en/newsletters/2025/12/12/#stratum-v2-updates)


### Criptografia e Segurança
* [A virtualized secure enclave for hardware signing devices](https://bitcoinops.org/en/newsletters/2025/12/12/#a-virtualized-secure-enclave-for-hardware-signing-devices)
* [ML-DSA based BIP-32 for lattices in post-quantum Bitcoin](https://x.com/roasbeef/status/1998549686463152229)
* [Why do different implementations produce different DER signatures for the same private key and hash?](https://bitcoin.stackexchange.com/questions/129270/why-do-different-implementations-produce-different-der-signatures-for-the-same-p)